...
public static partial class HttpApi {
    ...
    private static void SetupRoutes() {
        ...
        // Ticket routes
        AddRoute("GET", "/tickets", HandleGetTickets);
        AddRoute("POST", "/ticket/publish", HandlePublishTicket);
        AddRoute("POST", "/ticket/transfer-owner", HandleTransferOwnerTicket);
        AddRoute("POST", "/ticket/report", HandleReportTicket);
    }
    ...
    private static async Task HandleCreateContent(HttpListenerRequest request, HttpListenerResponse response) {
        ...
        if (contentType != "avatar" && contentType != "world" && contentType != "prop") {
    ...
    private static async Task HandleGetTickets(HttpListenerRequest request, HttpListenerResponse response) {
        if (TryAuthenticateRequest(request, response, out Account user)) return;

        List<object> tickets = DatabaseAccess.Execute(db => {
            ILiteCollection<Ticket> tickets = db.GetCollection<Ticket>("tickets");
            ILiteCollection<TicketMessage> messages = db.GetCollection<TicketMessage>("ticketMessages");

            return tickets.Find(t => t.UserId == user.Id).Select(t => new {
                type = t.Type,
                title = t.Title,
                status = t.Status,
                createdAt = t.CreatedAt,
                updatedAt = t.UpdatedAt,
                messages = messages.Find(m => m.TicketId == t.Id).OrderBy(m => m.CreatedAt).Select(m => new {
                    senderId = m.SenderId,
                    isStaff = m.IsStaff,
                    message = m.Message,
        ...
        await SendJsonResponse(response, tickets);
    }

    private static async Task HandlePublishTicket(HttpListenerRequest request, HttpListenerResponse response) {
        if (TryAuthenticateRequest(request, response, out Account user)) return;

        JObject? json = await GetRequestJson(request);
        if (json == null || !Guid.TryParse(json["contentId"]?.ToString(), out Guid contentId)) {
            await SendErrorResponse(response, "Invalid request format");
            return;
        }

        (bool success, string error) = DatabaseAccess.Execute(db => {
            Content? content = db.GetCollection<Content>("content").FindById(contentId);
            if (content == null || content.OwnerId != user.Id) {
                return (false, "Content not found or unauthorized");
            }

            if (db.GetCollection<Ticket>("tickets").Exists(t => t.ContentId == contentId && t.Type == "publish" && t.Status == "pending")) {
                return (false, "Content already has pending publish request");
            }

            Ticket ticket = new Ticket {
                UserId = user.Id,
                Type = "publish",
                Title = $"Publish Request: {content.Name}",
                Status = "pending",
                ContentId = contentId,
                CreatedAt = DateTime.UtcNow,
                UpdatedAt = DateTime.UtcNow
            ...
            TicketMessage message = new TicketMessage {
                ...
                SenderId = user.Id,
                IsStaff = false,
                Message = $"Requesting to publish {content.ContentType}: {content.Name}",
                CreatedAt = DateTime.UtcNow
            };

            db.GetCollection<Ticket>("tickets").Insert(ticket);
            db.GetCollection<TicketMessage>("ticketMessages").Insert(message);
    ...
    private static async Task HandleTransferOwnerTicket(HttpListenerRequest request, HttpListenerResponse response) {
        if (TryAuthenticateRequest(request, response, out Account user)) return;
       ...
       (bool success, string error) = DatabaseAccess.Execute(db => {
           ...
           if (db.GetCollection<Ticket>("tickets").Exists(t => t.ContentId == contentId && t.Type == "transfer-owner" && t.Status == "pending")) {
               return (false, "Content already has pending transfer request");
           }

           Ticket ticket = new Ticket {
               UserId = user.Id,
               Type = "transfer-owner",
               Title = $"Transfer Owner Request: {content.Name}",
               Status = "pending",
               ContentId = contentId,
               CreatedAt = DateTime.UtcNow,
               UpdatedAt = DateTime.UtcNow
           ...
           TicketMessage message = new TicketMessage {
               ...
               SenderId = user.Id,
               IsStaff = false,
               Message = $"Requesting ownership transfer of {content.ContentType}: {content.Name}",
               CreatedAt = DateTime.UtcNow
           };
    ...
    private static async Task HandleReportTicket(HttpListenerRequest request, HttpListenerResponse response) {
        if (TryAuthenticateRequest(request, response, out Account user)) return;

       MultipartFormDataParser? formData = await GetMultipartFormData(request);
       if (VerifyFields(response, formData)) return;

       string? type = formData.GetParameterValue("type");
       string? title = formData.GetParameterValue("title");
       string? description = formData.GetParameterValue("description");
       if (VerifyFields(response, type, title, description)) return;

       if (type != "user" && type != "avatar" && type != "world" && type != "prop") {
           await SendErrorResponse(response, "Invalid report type");
           return;
       }
